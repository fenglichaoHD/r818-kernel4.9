		...
		struct device_attribute *attr, char *buf)
{
	...
	struct nand_chip_info *chip = NULL;
	mutex_lock(nftl_blk->blk_lock);
	chip = aw_ndfc.nctri->nci;
		...
		struct device_attribute *attr, char *buf)
{
	...
	int ret = 0;
	mutex_lock(nftl_blk->blk_lock);
	ret = sprintf(buf, "badblock  cnt: %d\n", nftl_get_bad_block_cnt(nftl_blk));
	mutex_lock(nftl_blk->blk_lock);
	return ret;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	int ret = 0;
	mutex_lock(nftl_blk->blk_lock);
	ret = nftl_get_gc_info(nftl_blk->nftl_zone, buf, PAGE_SIZE);
	mutex_unlock(nftl_blk->blk_lock);
	return ret;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strcmp(cmd, "flush") == 0) {
		...
		nand_dbg_err("NAND DEBUG blk lock: %s %u\n", cmd, arg0_int);
		mutex_lock(nftl_blk->blk_lock);
		ret = gc_all(nftl_blk->nftl_zone, arg0_int);
		mutex_unlock(nftl_blk->blk_lock);
		return count;
