 ...
 * x >= 1; 00 <= ab <= 99
 */
#define NAND_COMMON1_PHY_DRV_VERSION "v1.07 2020-11-12 16:30-202109180934"
...
void *nand_dma_map_single(struct sunxi_ndfc *ndfc, __u32 rw, void *buff_addr, __u32 len, __u32 panic_res)
{
	void *mem_addr;
	...
	if (ndfc == NULL)
		nand_print("%s err: ndfc is null\n", __func__);
	if (is_on_panic())
		return (void *)nand_panic_dma_map(rw, buff_addr, len, panic_res);
	if (rw == 1) {
