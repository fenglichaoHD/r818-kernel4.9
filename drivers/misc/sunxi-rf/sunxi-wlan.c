...
#if defined EXTENDED_VCCWIFI_GPIO
	if (!on_off && gpio_is_valid(data->gpio_wlan_vccwifi_ctrl))
		gpio_set_value(data->gpio_wlan_vccwifi_ctrl, 0);
#endif
...
#if defined EXTENDED_VCCWIFI_GPIO
	if (on_off && gpio_is_valid(data->gpio_wlan_vccwifi_ctrl))
		gpio_set_value(data->gpio_wlan_vccwifi_ctrl, 1);
#endif
	if (on_off && gpio_is_valid(data->gpio_wlan_regon)) {
		mdelay(10);
#if defined(VIRTUAL_EXTENDED_GPIO)
...
#if defined EXTENDED_VCCWIFI_GPIO
	/* request gpio for ctrl vccwifi */
	data->gpio_wlan_vccwifi_ctrl = of_get_named_gpio_flags(np, "wlan_vccwifi_ctrl", 0,
							(enum of_gpio_flags *)
							&config);
	if (!gpio_is_valid(data->gpio_wlan_vccwifi_ctrl)) {
		dev_err(dev, "get gpio wlan_vccwifi_ctrl failed\n");
	} else {
		dev_info(dev, "wlan_vccwifi_ctrl gpio=%d  mul-sel=%d  pull=%d  drv_level=%d  data=%d\n",
				config.gpio,
				config.mul_sel,
				config.pull,
				config.drv_level,
		...
		ret = devm_gpio_request(dev, data->gpio_wlan_vccwifi_ctrl,
				"wlan_vccwifi_ctrl");
		if (ret < 0) {
			dev_err(dev, "can't request wlan_vccwifi_ctrl gpio %d\n",
				data->gpio_wlan_vccwifi_ctrl);
			goto end;
		}

		ret = gpio_direction_output(data->gpio_wlan_vccwifi_ctrl, 0);
		if (ret < 0) {
			dev_err(dev, "can't request output direction wlan_vccwifi_ctrl gpio %d\n",
				data->gpio_wlan_vccwifi_ctrl);
			goto end;
		}
	}
