...
static void usb_new_phy_init(struct sunxi_hci_hcd *sunxi_hci)
{
	...
	/*pll_vco_gain, val:0x07*/
	usb_new_phyx_tp_write(sunxi_hci, 0x1c, 0x07, 0x03);
	pr_debug("addr:%x,len:%x,value:%x\n", 0x1c, 0x03,
		usb_new_phyx_tp_read(sunxi_hci, 0x1c, 0x03));

	/*com_tune, val:0x0f*/
	usb_new_phyx_tp_write(sunxi_hci, 0x30, 0x0f, 0x0d);
	pr_debug("addr:%x,len:%x,value:%x\n", 0x30, 0x0d,
		usb_new_phyx_tp_read(sunxi_hci, 0x30, 0x0d));

	/*tx_tune, val:0x238a*/
	usb_new_phyx_tp_write(sunxi_hci, 0x60, 0x238a, 0x0e);
	pr_debug("addr:%x,len:%x,value:%x\n", 0x60, 0x0e,
		usb_new_phyx_tp_read(sunxi_hci, 0x60, 0x0e));

	/*mbias, val:0x2*/
	usb_new_phyx_tp_write(sunxi_hci, 0x92, 0x02, 0x03);
	pr_debug("addr:%x,len:%x,value:%x\n", 0x92, 0x03,
		usb_new_phyx_tp_read(sunxi_hci, 0x92, 0x03));

}

static void usb_new_phy_res_cal(struct sunxi_hci_hcd *sunxi_hci)
{
	int value;

	/*clear software res cail*/
	usb_new_phyx_tp_write(sunxi_hci, 0x43, 0x0, 0x01);
	usb_new_phyx_tp_write(sunxi_hci, 0x41, 0x0, 0x01);
	usb_new_phyx_tp_write(sunxi_hci, 0x40, 0x0, 0x01);
	/*res cail*/
	usb_new_phyx_tp_write(sunxi_hci, 0x40, 0x01, 0x01);
	mdelay(1);
	usb_new_phyx_tp_write(sunxi_hci, 0x41, 0x01, 0x01);

	while (1) {
		if (usb_new_phyx_tp_read(sunxi_hci, 0x42, 0x01))
			break;
	}

	/*set res*/
	value = usb_new_phyx_tp_read(sunxi_hci, 0x49, 0x04);
	pr_debug("addr:%x,,value:%x\n", 0x49, value);
	usb_new_phyx_tp_write(sunxi_hci, 0x44, value, 0x04);
	usb_new_phyx_tp_write(sunxi_hci, 0x41, 0x0, 0x01);
	usb_new_phyx_tp_write(sunxi_hci, 0x40, 0x0, 0x01);
	usb_new_phyx_tp_write(sunxi_hci, 0x43, 0x01, 0x01);
}
#endif
